...
const loadData = userData => {
    ...
    userData.forEach(data => {
        ...
        if (data.gender === 'male') {
            ...
            // <img class="p-1 w-50 h-50 rounded-full ring-2 ring-gray-300 dark:ring-gray-500" src='${data.picture.large}' alt="Bordered avatar">
            // <p>'${data.id.name}'</p>
            const imgCreate = `

                <div class="w-full max-w-sm bg-white rounded-lg mx-auto border border-gray-200 shadow-md dark:bg-gray-800 dark:border-gray-700 cols-4">
                <div class="flex justify-end px-4 pt-4">
                    ...
                    <!-- Dropdown menu -->
                    <div id="dropdown" class="hidden z-10 w-44 text-base list-none bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700 block" style="position: absolute; inset: 0px auto auto 0px; margin: 0px; transform: translate(455px, 83px);" data-popper-reference-hidden="" data-popper-escaped="" data-popper-placement="bottom">
                ...
                <div class="flex flex-col items-center pb-10">
                    <img class="mb-3 w-24 h-24 rounded-full ring-2 ring-gray-300 dark:ring-gray-500 shadow-lg" src="${data.picture.large}" alt="Bonnie image">
                    <h5 class="mb-1 text-xl font-medium text-gray-900 dark:text-white">${data.name.title} ${data.name.first} ${data.name.last}</h5>
                    <span class="text-sm text-gray-500 dark:text-gray-400">${data.location.state}, ${data.location.country}</span>
